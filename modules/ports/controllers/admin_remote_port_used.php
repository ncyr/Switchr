<?php
class Admin_Remote_port_used extends Admin_Controller {
	protected $section = 'remote_port_used';
	...
	function index()
	{
		$extra['title'] = 'port:remote_port_used';
	 
		$extra['buttons'] = array(
		    array(
			'label'     => lang('global:edit'),
			'url'       => 'admin/ports/remote_port_used/edit/-entry_id-'
		    ),
		    array(
			'label'     => lang('global:delete'),
			'url'       => 'admin/ports/remote_port_used/delete/-entry_id-',
			'confirm'   => true,
		    )
		);
		 
		$this->streams->cp->entries_table('remote_port_used', 'ports', 3, 'admin/ports/remote_port_used/index', true, $extra);
	}
	function create(){
		
	    $extra = array(
		'return' => 'admin/ports/remote_port_used/index',
		'success_message' => lang('global:submit_success'),
		'failure_message' => lang('global:submit_failure'),
		'title' => 'lang:ports:new',
	     );
    
	    $this->streams->cp->entry_form('remote_port_used', 'ports', 'new', null, true, $extra);
	}
	...
	public function edit($id)
	{
		...
		$this->streams->cp->entry_form('remote_port_used', 'ports', 'edit', $id, true, $extra);
	}
