...
<script type="text/javascript">
    Stripe.setPublishableKey('<?php echo Settings::get('payignite_setting_public_key')?>');
</script>
...
<div style="padding-left:1em;">
    ...
    <div style="display:block;">
        ...
        <p id="total" style="display:inline;">$<?php echo $_POST['total_amount']; ?></p>
    </div>
</div>
...
<form action="" method="POST" id="payment-form" class="form-horizontal">
    ...
    <input type="hidden" name="coupon" value="<?php echo strtoupper($_POST['coupon']); ?>">
...
<form action="" method="POST" id="plan-form" class="form-horizontal">
    ...
    <div class="form-group">
        ...
        <label for="coupon" class="col-sm-4 control-label">Coupon: </label>
        <div class="col-sm-8">
            <input id="coupon" type="text" size="10" name="coupon" class="form-control" style="width:initial;display:inline-block;">
            <button id="apply-coupon" class="btn btn-info form-control" style="width:initial;display:inline-block;margin-left:20px;">Apply</button>
        </div>
    ...
    <div class="form-group">
        <div class="col-sm-offset-4 col-sm-8">
            <input type="submit" class="submit btn btn-primary" value="Pay for Plan" style="width:initial;">
        </div>
    </div>
</form>
...
$( document ).ready(function() {
    $('#total').text($('#hosts').val() * 25);
    $('#total_amount').val($('#hosts').val() * 25);
});

$('.amount').on('click keyup change', function() {
    var hosts = $('#hosts').val() * 25;
    // var s3 = $('#s3').val() * 5;
    // var ftp = $('#ftp').val() * 5;
...
$('#apply-coupon').click(function(e) {
    e.preventDefault();
    $.ajax({
        url: '/payignite/ajaxCoupon',
        type: 'POST',
        data: {
            coupon: $('#coupon').val(),
            subtotal: $('#hosts').val() * 25
        },
    }).done(function(response) {
        // If Stripe doesn't return an error.
        if (response.charAt(0) !== '{') {
            $('#total').text(response);
            $('#total_amount').val(response);
        } else {
            $('#total').text($('#hosts').val() * 25);
            $('#total_amount').val($('#total').text($('#hosts').val() * 25));
        }
    });
});
</script>
