...
if ($this->input->post('submit')) {
    ...
    if ($this->input->post('customer_plan') == 'none') {
            ...
            // User Data
            $cu->email = $this->input->post('email');
        $cu->account_balance = $account_balance;
        $cu->coupon = $coupon;

            // CC Data
            $cu->cards->data['0']->name = $this->input->post('card_name');
            //$cu->cards->data['0']->number    = $card_number;
            //$cu->cards->data['0']->cvc       = $this->input->post('card_cvc');
            //$cu->cards->data['0']->exp_month = $this->input->post('card_exp_month');
            //$cu->cards->data['0']->exp_year  = $this->input->post('card_exp_year');
            $cu->cards->data['0']->address_line1 = $this->input->post('card_add1');
        $cu->cards->data['0']->address_line2 = $this->input->post('card_add2');
        $cu->cards->data['0']->address_city = $this->input->post('card_city');
        $cu->cards->data['0']->address_state = $this->input->post('card_state');
        $cu->cards->data['0']->address_zip = (string) $this->input->post('card_zip');
    } else {

            // User Data
            $cu->email = $this->input->post('email');
        $cu->account_balance = $account_balance;
        $cu->plan = $this->input->post('customer_plan');
        $cu->quantity = $plan_quantity;
        $cu->trial_end = (int) $trial_end;
        $cu->coupon = $coupon;
...
//Stripe_Customer::retrieve($customer['id']); ?>


<section class="title">
    <h4><?php echo lang('payignite:customer_edit'); ?></h4>
</section>
...
<div class="content">
    ...
    <thead>
        ...
        <tbody>
            <tr>
                <td>
                    <?php //echo print_r($customer);
                    //echo print_r($customer['subscriptions']['data']);
                    echo form_input('email', $cu->email); ?>
                </td>

                <td>$
                    <?php $account_balance = array('name' => 'account_balance', 'size' => '5', 'value' => $cu->account_balance);
                    echo form_input($account_balance); ?>
                </td>

                <td>
                <?php
                    $plan['entries'] = \Stripe\Plan::all();
                    $plan_options = array('none' => 'No Plan');
                    foreach ($plan['entries']['data'] as $row) {
                        $plan_options[$row->id] = $row->name;
                    }
                    $plan_selected = $customer['subscriptions']['data'] == false ? 'none' : $customer['subscriptions']['data']['0']['plan']['id'];
                    echo form_dropdown('customer_plan', $plan_options, $plan_selected); ?>
                </td>
    ...
    <table class="table" cellpadding="0" cellspacing="0">
        ...
        <tr>
        <td><?php $card_name_data = array('name' => 'card_name', 'class' => '.card-name', 'value' => $cu->cards->data['0']->name);
                    echo form_input($card_name_data); ?>
                </td>

                <td><?php $card_number_data = array('name' => 'card_number', 'class' => '.card-number', 'value' => 'XXXX-XXXX-XXXX-'.$cu->cards->data['0']->last4);
                    echo form_input($card_number_data); ?>
                </td>

                <td><?php $cvc_data = array('name' => 'card_cvc', 'size' => '3', 'class' => '.card-cvc', 'value' => $cu->cards->data['0']->cvc_check);
                    echo form_input($cvc_data); ?>
                </td>

                <td>
                    <?php $month_data = array('name' => 'card_exp_month', 'size' => '3', 'placeholder' => 'mm', 'class' => '.card-exp-month', 'value' => $cu->cards->data['0']->exp_month);
                    echo form_input($month_data); ?>
                    /
                    <?php $year_data = array('name' => 'card_exp_year', 'size' => '3', 'placeholder' => 'yy', 'class' => '.card-exp-year', 'value' => $cu->cards->data['0']->exp_year);
    ...
    <thead>
        ...
        <tr>
        <td><?php $card_add1_data = array('name' => 'card_add1', 'class' => '.card-add1', 'value' => $cu->cards->data['0']->address_line1);
                    echo form_input($card_add1_data); ?>
                </td>

                <td><?php $card_add2_data = array('name' => 'card_add2', 'class' => '.card-add2', 'value' => $cu->cards->data['0']->address_line2);
                    echo form_input($card_add2_data); ?>
                </td>

                <td><?php $card_city_data = array('name' => 'card_city', 'class' => '.card-city', 'value' => $cu->cards->data['0']->address_city);
                    echo form_input($card_city_data); ?>
                </td>

                <td><?php $card_state_data = array('name' => 'card_state', 'class' => '.card-state', 'value' => $cu->cards->data['0']->address_state);
                    echo form_input($card_state_data); ?>
                </td>

                <td><?php $card_zip_data = array('name' => 'card_zip', 'class' => '.card-zip', 'value' => $cu->cards->data['0']->address_zip);
